...
// =============================================================================
// Redirigir TODOS los QR de un local a la vez
// =============================================================================

import { useRef, useState, useTransition } from 'react';
import { redirigirLote } from '@/lib/acciones/qr';

export function RedirigirLote({ accountId, cuenta }: { accountId: string; cuenta: string }) {
    ...
    <>
      ...
      <dialog ref={dialogo} className="superficie w-[min(26rem,92vw)] rounded-xl border borde-tema p-0 backdrop:bg-black/60">
        <form onSubmit={enviar} className="p-5">
          <h2 className="mb-1 text-base font-semibold">Redirigir todos los QR de {cuenta}</h2>
          <p className="texto-suave mb-4 text-xs">
            Afecta a todas las mesas de este local de una sola vez. Déjalo vacío para
            devolver todas al flujo normal de escaneo.
          </p>

          <label className="texto-suave mb-1 block text-xs font-medium" htmlFor="destinoUrlLote">URL de destino (https://)</label>
          <input
            id="destinoUrlLote" name="destinoUrl" type="url" placeholder="https://…"
            className="superficie w-full rounded-md border borde-tema px-3 py-2 text-sm"
          />

          <label className="texto-suave mb-1 mt-3 block text-xs font-medium" htmlFor="campanaLote">Campaña</label>
          <input
            id="campanaLote" name="campana"
            className="superficie w-full rounded-md border borde-tema px-3 py-2 text-sm"
          />
          ...
          <div className="mt-5 flex justify-end gap-2">
            ...
            <button type="submit" disabled={enCurso} className="rounded-md bg-orange-600 px-4 py-2 text-sm font-medium text-white hover:bg-orange-500 disabled:opacity-50">
              {enCurso ? 'Guardando…' : 'Aplicar a todos'}
            </button>
          </div>
        </form>
      </dialog>
