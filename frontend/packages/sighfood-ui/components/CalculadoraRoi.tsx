/**
 * ============================================================================
 * CALCULADORA ROI UI — SRP: Responsabilidad Única de Presentación
 * ============================================================================
 * 
 * PRINCIPIO SRP (Capítulo 7):
 * ───────────────────────────────────────────────────────────────────────────
 * Este componente tiene UNA sola razón para cambiar: si el equipo de Diseño/UX
 * modifica cómo se ve la calculadora (colores, disposición, tipo de slider).
 * 
 * NO cambia por:
 *   • Cambios en la fórmula de ROI (vive en calcularRoi.ts del dominio)
 *   • Cambios en validación de formularios (vive en validarFormularioLead.ts)
 *   • Cambios en cómo se envían los datos al CRM (responsabilidad del Use Case)
 * 
 * Este componente SOLO:
 *   • Captura la entrada del usuario (tragos por fin de semana)
 *   • Invoca la función pura del dominio
 *   • Renderiza el resultado
 * 
 * REFERENCIAS DEL LIBRO:
 *   • Capítulo 7: SRP — Principio de Responsabilidad Única
 *   • Capítulo 6: Programación Funcional — segregar mutabilidad
 * ============================================================================
 */

'use client';

import { useState } from 'react';
import { calcularRoiMensual } from '../../sighfood-domain/rules/calcularRoi';

export function CalculadoraRoi() {
  const [tragos, setTragos] = useState(100);
  
  // Invocar función pura del dominio — sin lógica de negocio aquí
  const resultado = calcularRoiMensual(tragos);

  const formatCurrency = (amount: number) => {
    return new Intl.NumberFormat('es-CO', {
      style: 'currency',
      currency: 'COP',
      minimumFractionDigits: 0,
      maximumFractionDigits: 0,
    }).format(amount);
  };

  return (
    <div className="bg-[#1f1f1f] border border-gray-800 rounded-lg p-8 md:p-12">
      <div className="text-gray-500 text-sm mb-2 tracking-widest uppercase">
        Calculadora de Margen
      </div>
      
      <h2 className="text-3xl md:text-4xl font-bold mb-8 text-[#f5f5f5]">
        ¿Cuánto puede ganar tu barra?
      </h2>
      
      <div className="mb-8">
        <div className="flex justify-between items-center mb-4">
          <label className="text-gray-300 font-medium">
            Tragos servidos por fin de semana
          </label>
          <span className="text-[#d97325] text-2xl font-bold">{tragos}</span>
        </div>
        
        <input
          type="range"
          min="10"
          max="1000"
          step="10"
          value={tragos}
          onChange={(e) => setTragos(Number(e.target.value))}
          className="w-full h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-[#d97325]"
        />
        
        <div className="flex justify-between text-xs text-gray-500 mt-2">
          <span>10</span>
          <span>500</span>
          <span>1000</span>
        </div>
      </div>
      
      <div className="border-t border-gray-800 pt-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-[#0f0f0f] rounded-lg p-6 border border-gray-800">
            <div className="text-xs text-gray-500 uppercase tracking-wider mb-2">
              Conos estimados por mes
            </div>
            <div className="text-3xl md:text-4xl font-bold text-[#d97325]">
              {resultado.conosEstimados}
            </div>
          </div>
          
          <div className="bg-[#0f0f0f] rounded-lg p-6 border border-gray-800">
            <div className="text-xs text-gray-500 uppercase tracking-wider mb-2">
              Ganancia mensual proyectada
            </div>
            <div className="text-3xl md:text-4xl font-bold text-[#f5f5f5]">
              {formatCurrency(resultado.gananciaNetaMensualCOP)}
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}